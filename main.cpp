...
int main(int argc, char* argv[])
{
	...
	do
	{
		...
		//Get a console handle (using <Windows.h>)
		AllocConsole();
		HWND console = GetConsoleWindow();

		//Get a handle to device context
		HDC deviceContent = GetDC(console);
		HDC* ptrDevice = &deviceContent;
		...
		*/
		...
		oUlam.setScreenPath(lValue, oPixelDrawing, ptrDevice, &lValue);

		ReleaseDC(console, deviceContent);
