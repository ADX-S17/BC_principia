...
int Ulam::setScreenPath(long lLast, PixelDrawing* oPixelDrawing, HDC* deviceContent, long* maxValue)
{
	bool bRepere = true;
	...
	for (long lNombreCase = 1; lNombreCase <= lLast; lNombreCase++)
	{
		...
		if (bRepere)
		{
			oPixelDrawing->goRight(lNombreCase, &lInitValue, &m_vPrimeNumbers, deviceContent, maxValue);
			if (lInitValue >= lLast) break;
			oPixelDrawing->goUp(lNombreCase, &lInitValue, &m_vPrimeNumbers, deviceContent, maxValue);
			if (lInitValue >= lLast) break;
			bRepere = false;
		}
		else
		{
			oPixelDrawing->goLeft(lNombreCase, &lInitValue, &m_vPrimeNumbers, deviceContent, maxValue);
			if (lInitValue >= lLast) break;
			oPixelDrawing->goDown(lNombreCase, &lInitValue, &m_vPrimeNumbers, deviceContent, maxValue);
			if (lInitValue >= lLast) break;
			bRepere = true;
		}		
	}
